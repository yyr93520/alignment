...
def model_test(test_labels, embeddings_src, embeddings_obj, res_file):
    ...
    for label_src, label_obj in test_labels:
        ...
        src_vec = tf.placeholder('float', [1, n_input])
        obj_vec = tf.placeholder('float', [1, n_out])
...
def train_model(source_file, target_file, train_label_file, test_label_file, result_file):
    # Construct model
    ...
    with tf.Session() as sess:
        ...
        embeddings_src = read_embeddings(source_file)
        embeddings_obj = read_embeddings(target_file)
        train_labels = read_identity_labels(train_label_file)
        test_labels = read_identity_labels(test_label_file)
        ...
        for epoch in range(training_epochs):
            ...
            # Test model
            model_test(test_labels, embeddings_src, embeddings_obj, result_file)
...
if __name__ == '__main__':
    ...
    embedding_pt = config.get('FEATURE', 'embedding_pt')
    source_file = '%s/%s_%s.txt' % (embedding_pt, 'Douban', '1st')
    target_file = '%s/%s_%s.txt' % (embedding_pt, 'Weibo', '1st')
    train_pt = config.get('FEATURE', 'multi_train_pt')
    train_label = '%s/embedding_train.txt' % (train_pt)
    test_label = '%s/embedding_test.txt' % (train_pt)
    result_file = '%s/result.txt' % (train_pt)
    train_model(source_file, target_file, train_label, test_label, result_file)
